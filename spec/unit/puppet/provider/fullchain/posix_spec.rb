...
describe 'The POSIX provider for type fullchain' do
  before(:each) do
  ...
  describe 'POSIX provider for fullchain type' do
    ...
    let(:user_pem) { @user_crt.to_pem } # rubocop:disable RSpec/InstanceVariable
    let(:fullchain_pem) { @user_crt.to_pem + @int_crt.to_pem + @root_crt.to_pem } # rubocop:disable RSpec/InstanceVariable

    it 'exists? should return false if fullchain file does not exist' do
